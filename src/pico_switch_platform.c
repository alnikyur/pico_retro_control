...
static void pc_power_control_init(void) {
    ...
    gpio_pull_down(POWER_BTN_GPIO); // Защита от наводка при загрузке
...
static bool press_power_button_safe(void) {
    // Если на PLED+ есть напряжение (1 = ПК включен или уже запускается)
    if (gpio_get(PC_SENSE_GPIO) == 1) {
        logi("my_platform: PC is already ON, skipping power button press\n");
        return false;
    }

    // ПК выключен -> подаем короткий импульс 300 мс на базу C1815
    logi("my_platform: PC is OFF, pressing power button\n");
    gpio_put(POWER_BTN_GPIO, 1); // Открываем транзистор
    sleep_ms(300);
    gpio_put(POWER_BTN_GPIO, 0); // Закрываем транзистор
...
static uni_error_t pico_switch_platform_on_device_ready(uni_hid_device_t* d) {
    ...
    // Вызываем пульс и проверку включения ПК
